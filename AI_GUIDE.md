...
- 外部検索: フロントエンドの provider 境界から Google Books API を利用。openBD 追加用の provider 枠あり
...
- `src/libs/bookSearch/*`: 外部書籍検索 provider。現状は Google Books が実装済み、openBD は追加用の枠のみ
...
- `src/libs/bookSearch/providers/openBd.ts` は openBD 追加用の placeholder。
...
- openBD provider は現状未実装。ISBN検索 fallback として追加するのが自然。
