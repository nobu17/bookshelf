...
- 外部検索: フロントエンドから国立国会図書館サーチの OpenSearch API を利用
...
- `src/libs/apis/bookSearch.ts` が `https://ndlsearch.ndl.go.jp/api/opensearch` を直接呼ぶ。
- XML を `fast-xml-parser` で parse し、ISBN10 は ISBN13 に変換する。
